...
async function checkRateLimit(uid: string): Promise<void> {
  const ref = db.doc(`users/${uid}/rateLimits/analyze`);
  ...
  if (snap.exists) {
    ...
    if (now - windowStart < RATE_LIMIT_WINDOW_MS) {
      if (data.count >= RATE_LIMIT_MAX) {
        logger.warn('Rate limit exceeded', { uid, count: data.count });
        throw new HttpsError(
          'resource-exhausted',
          'Rate limit exceeded. Max 10 analyses per hour.'
        );
      }
  ...
  // New window
  await ref.set({ count: 1, windowStart: Timestamp.fromMillis(now) });
}
...
export const createCheckoutSession = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      logger.info('Checkout session created', { uid, sessionId: session.id });
      return { url: session.url };
    } catch (error: any) {
      logger.error('createCheckoutSession failed', { uid, error: error.message });
    ...
    try {
      if (event.type === 'checkout.session.completed') {
        ...
        if (uid) {
          ...
          logger.info('User upgraded to Pro', { uid, customerId: session.customer });
        }
      }
      ...
      if (event.type === 'customer.subscription.deleted') {
        ...
        if (!usersSnap.empty) {
          ...
          logger.info('User downgraded to Free', { uid, customerId });
        }
      }

      res.status(200).json({ received: true });
    } catch (error: any) {
      logger.error('stripeWebhook processing error', { eventType: event.type, error: error.message });
...
export const deleteUserAccount = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      // Delete rate limit docs
      const rateLimitsRef = db.collection(`users/${uid}/rateLimits`);
      const rateLimitsSnap = await rateLimitsRef.listDocuments();
      for (const docRef of rateLimitsSnap) {
        batch.delete(docRef);
      }
...
// ─────────────────────────────────────────────────────────────
export const getUploadToken = onCall(
  { timeoutSeconds: 60 },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Authentication required.');
    ...
    try {
      ...
      logger.error('getUploadToken failed', { uid, error: error.message });
      throw new HttpsError('internal', `Upload token generation failed: ${error.message}`);
    }
  }
);
...
export const analyzeDocument = onCall(
  ...
  async (request) => {
    ...
    // ── Rate limiting: max 10 calls per uid per hour ──
    ...
    logger.info('Analysis started', { uid, docId, fileType });
    ...
    try {
      ...
      logger.error('analyzeDocument failed', { uid, docId, error: errorMessage });
